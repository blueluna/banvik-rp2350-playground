...
async fn buttons_task(controls: &'static mut Controls<'static>) -> ! {
    ...
    loop {
        ...
        let _ = controls.led_1.set_duty_cycle(button_level.0);
        let _ = controls.led_2.set_duty_cycle(button_level.1);
        let _ = controls.led_3.set_duty_cycle(button_level.2);
        let _ = controls.led_4.set_duty_cycle(button_level.3);
        ticker.next().await;
    }
}
